...
% Constructs surrogates over a small region within a larger parameter space
% to see how accurate the local approximation is, and how bad the global
% approximation is.  I compare two approaches, a surrogate trained on
...
ss = sobolset(2);
randomSampler = @(nSamples) mvnrnd([8,12], 0.5*eye(2), nSamples);
...
outputs = BraninFunction(testRVS);                                         
...
gp = fitrgp(testRVS, outputs);
ZZZ2 = zeros(100);
for iX = 1:100
    for iY = 1:100
        ZZZ2(iX, iY) = predict(gp, [x(iX), y(iY)]);
    end
end
surf(XXX, YYY, ZZZ2);
hold on
