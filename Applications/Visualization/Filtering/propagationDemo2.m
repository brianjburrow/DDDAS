...
clc
%% propagationDemo2.m
% This function is designed to show the challenge of propagating a
% distribution through a nonlinear function.  First, Gaussian distributions
% do not remain gaussian after a nonlinear transformation.  Second, there
% is no closed form solution to propagating an entire distribution through
...
function state = stateEvo(state, slope, add)
    state = slope * abs(state).^1.5 - 20 - add;
end
