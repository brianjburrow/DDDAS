...
for iIter = 1:length(numIter)
    ...
    %         propSamples = propSamples(indx,:); 
             [weights, propSamples, EFF] = determineWeights(propSamples, samples, iter);
            %disp(EFF)
    ...
    figure(1)
    subplot(1,1,1)
    plot(sampleSize, mean(moment12), 'LineWidth', 2.0)
    hold on
    ...
    figure(2)
    subplot(1,1,1)
    plot(sampleSize, mean(moment13), 'LineWidth', 2.0)
    hold on
...
function [weights, samples, EFF] = determineWeights(samples, newSamples, itr)
    %% Determine Weights
