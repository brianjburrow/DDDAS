...
for iTime = 2:Tmax
    ...
    subplot(3,2, 3)
    [muVec, stdVec] = dynGP2.predict(libraryIn);
    niceErrorbar(libraryIn, muVec, 3*stdVec)
    hold on
    ylim([0, 100])
    xlim([-40, 40])
    xlabel('X')
    ylabel('f(X)')
    scatter(dynGP2.trainInput, dynGP2.trainOutput, 10, 'filled', 'b')
    drawnow
    hold off
    ...
    if iTime > 1
        ...
        subplot(3, 2, 4)
        niceErrorbar([1:iTime - 1]', zeros([iTime - 1, 1]), pf2Err(1:iTime - 1)');
        hold on
        scatter(1:iTime - 1, states(1:iTime - 1) - pf2Est(1:iTime - 1), 10, 'filled', 'r')
        ylim([-40, 40])
        xlim([0, 70])
        xlabel('Time')
        ylabel('Error')
