...
function ok = TestNoninformativeBayesUpdate()
for i=1:15
for j=1:15
...
discrepancy1 = max(abs(mu_withsqrt'-mu_prediction));
discrepancy2 = max(abs(mu_withsqrt'-mu'));
discrepancy3 = max(abs(diag(A_withsqrt)-var_prediction'));
discrepancy4 = max(abs(diag(A_withsqrt)-diag(A)));

disp(sprintf('discrepancy=%g between NoninformativeBayesUpdateSqrt and the theoretical posterior mean', discrepancy1));
disp(sprintf('discrepancy=%g between NoninformativeBayesUpdateSqrt and NoninformativeBayesUpdate posterior mean', discrepancy2));
disp(sprintf('discrepancy=%g between NoninformativeBayesUpdateSqrt and the theoretical posterior variance', discrepancy3));
disp(sprintf('discrepancy=%g between NoninformativeBayesUpdateSqrt and NoninformativeBayesUpdate posterior variance', discrepancy4));

if (max([discrepancy1,discrepancy2,discrepancy3,discrepancy4]) < 1e-10)
	ok = 1;
	disp('TestNoninformativeBayesUpdate: OK');
else
	ok = 0;
	disp('TestNoninformativeBayesUpdate: FAIL');
end
