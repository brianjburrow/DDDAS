...
% hyperparameters.
%
% This is version 4 of the algorithm, with 3 previous versions in obsolete/.
%
...
function [impl,x,y,extras] = SimFixedHyperCKG(N,sample,dim,n0,p,MPerDim,mu0,cov0,scale,noisevar)
	...
	if (noisevar == 0)
		% Correct SigmaSqrt for perfect measurements.
		SigmaSqrt(x(1:n-1),:)=0;
		SigmaSqrt(:,x(1:n-1))=0;
	end
    	[x(n),extras.logkgfactor(n)] = CorrelatedKG(mun,SigmaSqrt,lambda);
	xd(n,:) = toZd(x(n));
	y(n) = sample(x(n));
