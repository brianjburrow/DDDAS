%[impl,x,y,extras] = SimAdaptiveNoNoiseEGO(N,sample,dim,n0,MPerDim,scale0)
%
% Simulates EGO algorithm through N samples, and collects at each sample time
% 1<=n<=N the measurement and implementation decision.  Assumes 0 noise, and
% adaptively estimates the hyperparameters on each iteration.  The previous MLE
% estimate of the hyperparameters is used as the starting point for the next
% iteration.
%
...
%
function [impl,x,y,extras] = SimAdaptiveNoNoiseEGO(N,sample,dim,n0,MPerDim,scale0)
    M = MPerDim^dim;
    ...
    impl(n) = Argmax(mun);
    disp(sprintf('n=n0=%d: EGO x=%d (%s) impl=%d (%s) mun[impl]=%f scale=%s cov0=%g mu0=%g', ...
	...
	sposterior(x(1:n-1))=0; % Correct for perfect measurements
	...
	impl(n) = Argmax(mun);
	disp(sprintf('n=%d: EGO x=%d (%s) impl=%d (%s) mun[impl]=%f scale=%s cov0=%g mu0=%g', ...
		n, x(n), mat2str(xd(n,:)), impl(n), mat2str(toZd(impl(n))), ...
		mun(impl(n)), mat2str(extras.scale(:,n),3), extras.cov0(n), ...
		extras.mu0(n))); 
    end
