...
function [cov0hat, noisevarhat, scalehat, betahat, dmodel, pv] = EstimateHyperparameters(xd, y, scale0, scaleL, scaleU, g0, gL, gU)
	...
	% Transform coordinates to theta to be passed to corrgaussnoise.
	theta0 = [scale0.^-2 g0];
	thetaL = [scaleU.^-2 gL];
	thetaU = [scaleL.^-2 gU];
	...
	theta=theta0;
	for i=1:ntries
		[dmodel,pv]=mydacefit(xd,y,@regpoly0,@corrgaussnoise,theta,thetaL,thetaU);
		%{
