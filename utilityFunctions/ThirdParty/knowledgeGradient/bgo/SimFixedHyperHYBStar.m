%[impl,x,y,extras] = SimFixedHyperHYBStar(N,sample,dim,n0,p,MPerDim,mu0,cov0,scale,noisevar)
%
% Simulates the hybrid KG(*) algorithm through N samples, and collects at each
% sample time 1<=n<=N the measurement and implementation decision.  Uses fixed
% hyperparameters.
%
...
%
function [impl,x,y,extras] = SimFixedHyperHYBStar(N,sample,dim,n0,p,MPerDim,mu0,cov0,scale,noisevar)
    if (p~=2)
	...
	nleft = N-n+1;
    	[x(n),extras.logkgfactor(n)] = IndKGStar_TangentPoint(mun,SigmaDiag,noisevar,nleft);
	xd(n,:) = toZd(x(n));
	y(n) = sample(x(n));
