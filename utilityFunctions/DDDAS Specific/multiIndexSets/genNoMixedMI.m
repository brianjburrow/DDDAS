function multi_indices = genNoMixedMI(order, dimensions)
...
function binaryCheck = checkSetInclusion(VECTOR, order, dim)
    ...
    if (magSum <= order) && (magSum >= 0)
        ...
        for idx = 1:length(VECTOR(1,:))
            for dmx = 1:length(VECTOR(1,:))
                if (VECTOR(1, idx)*VECTOR(1, dmx) ~= 0) && (binaryCheck) && (idx ~= dmx)
                    binaryCheck = false;
                elseif (idx > dim) && (VECTOR(1, idx) ~= 0)
                    binaryCheck = false;
                end
            end
        end
